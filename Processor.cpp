...
Processor::Processor() : m_shut_down_flag(0) {}
...
void Processor::TestRoutine(int cmd) {
    FUNC_WARDER;
...
void Processor::TestPrice(int cmd, double* open_price, double* close_price, double* sl, double* tp) {
    double prices[2];
...
void Processor::TestEntry(LPVOID parameter) {
    for (int cmd = OP_BUY; cmd <= OP_SELL_STOP; cmd++) {
        ((Processor*)parameter)->TestRoutine(cmd);
    }
    _endthread();
}

void Processor::UnitTest() {
    _beginthread(TestEntry, 0, this);
}

bool Processor::OpenOrder(const int login, const char* ip, const char* symbol, const int cmd, int volume, double open_price,
...
bool Processor::AddOrder(const int login, const char* ip, const char* symbol, const int cmd, int volume, double open_price,
                         double sl, double tp, const char* comment, char message[MESSAGE_MAX_SIZE], int* order) {
    FUNC_WARDER;
...
bool Processor::UpdateOrder(const char* ip, const int order, double open_price, double sl, double tp, const char* comment,
                            char message[MESSAGE_MAX_SIZE]) {
    FUNC_WARDER;
...
bool Processor::CloseOrder(const char* ip, const int order, double close_price, const char* comment,
                           char message[MESSAGE_MAX_SIZE]) {
    FUNC_WARDER;
...
bool Processor::SetBalance(const int login, const char* ip, const double value, const char* comment,
                           char message[MESSAGE_MAX_SIZE]) {
    FUNC_WARDER;
    ...
    if (Factory::GetServerInterface()->ClientsChangeBalance(login, &user.grp, value, comment) == 0) {
        return false;
    }
    return true;
}

bool Processor::GetUserInfo(const int login, UserInfo* user_info) {
...
bool Processor::GetCurrentPrice(const char* symbol, const char* group, double* prices) {
    ConGroup grpcfg = {0};
